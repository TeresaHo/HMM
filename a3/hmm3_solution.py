...
class MyHmmScaled(object):
    ...
    def __init__(self):
        ...
        #with open("hmm4_03.in", "r") as inputs:
        f = 0
        for line in sys.stdin:
            ...
            if f == 1:
                ...
                col = int(line[1])
                line = line[2:]
                self.M = col # number of observation
                B = {}
                for i in range(self.N):
                    B[i] = {}
                ...
                line = line[2:]
                pi = {}
                for num,item in enumerate(line,0):
                    pi[num] = float(item)
                self.pi = pi
                
            elif f==4:
                line = line.split()
                self.num_seq = int(line[0])
                line = line[1:]
                self.obs = []
                for oi in line:
                    self.obs.append(int(oi))
    ...
    def backward_scaled(self, obs):
        ...
        for t in reversed(range(T-1)):
            beta_local = {}
            for y in range(self.N):
                beta_local[y] = sum((self.bwk_scaled[t+1][y1] * self.A[y][y1] * self.B[y1][obs[t+1]]) for y1 in range(self.N))
    ...
    def forward_scaled(self, obs):
        ...
        c1 = self.compute_cvalue(self.fwd[0])
        ...
        for t in range(1, len(obs)):
            ...
            for y in range(self.N):
                ...
                local_alpha[y] = sum((self.fwd_scaled[t-1][y0] * self.A[y0][y] * self.B[y][obs[t]]) for y0 in range(self.N))
                #if (local_alpha[y] == 0):
                    #print ("ERROR local alpha is zero: y = ", y)
                    #print ("fwd = %3f, A = %3f, B = %3f, obs = %s" % (self.fwd_scaled[t - 1][y0], self.A[y0][y], self.B[y][obs[t]], obs[t]))

            c1 = self.compute_cvalue(local_alpha)
            self.clist.append(c1)
            # create scaled alpha values
            for y in range(self.N):
    ...
    def forward_backward_multi_scaled(self, obs): # returns model given the initial model and observations
        count = 40
        for iteration in range(count):
            tables = self.create_zi_gamma_tables(obs)
            zi_table = tables["zi_table"] # this will have zi values [t-1][i][j]
            gamma_table = tables["gamma_table"] # this will have gamma values [t][i]           
            ...
            for i in range(self.N):
                ...
                for sym in range(self.M):
                    temp_bjk[i][sym] = self.compute_bj(tables, i, obs, sym)
                for j in range(self.N):
            ...
            ## re pi
            for i in range(self.N):
                temp_pi[i] = gamma_table[0][i]
            ### re A
            for i in range(self.N):
                temp_aij[i] = {}
                de = 0.0
                for t in range(len(zi_table)):
                    de = de + gamma_table[t][i]
                for j in range(self.N):
                    nu = 0.0
                    for t in range(len(zi_table)):
                        nu = nu + zi_table[t][i][j]
                    temp_aij[i][j] = nu / de
            ### re B
            for i in range(self.N):
                temp_bjk[i] = {}
    ...
    def compute_aij(self, tables, i, j):
        ...
        for t in range(len(zi_table) ): # sum over all t up to Tk - 1
            denominator += gamma_table[t][i] # zi value for i, j
            numerator += zi_table[t][i][j] # zi value for i, j
    ...
    def compute_bj(self, tables, i, obslist, symbol):
        ...
        for t in range(len(gamma_table) ): # sum over all t up to Tk - 1
            denominator += gamma_table[t][i] # zi value for i, j
            if obslist[t] == symbol:
                numerator += gamma_table[t][i] #zi_table[k][t][i][j] # zi value for i, j
    ...
    def compute_pi(self, tables, i):
        ...
        pi = 0.0
         # sum over all observations in the multi list
        pi += gamma_table[0][i] #zi_table[k][t][i][j] # zi value for i, j
        return pi
    ...
    def create_zi_gamma_tables(self, obs):
        ...
        for t in range(len(obs)-1): # 1 <= t <= Tk - 1
            ...
            for i in range(self.N):  
                ...
                for j in range(self.N):
                    ...
                    gamma_t[t][i] = gamma_t[t][i] + zi_t[t][i][j]
        gamma_t.append({})
        for i in range(self.N):
            gamma_t[t+1][i] = self.fwd_scaled[t+1][i]
        zi_table = zi_t
        gamma_table = gamma_t
...
if __name__ == '__main__':
    ...
    model = MyHmmScaled()
    aij, bjk, i = model.forward_backward_multi_scaled(model.obs)
    ...
    for k in aij:
        for x in aij[k].values():
            ans_A.append(x)

    for k in bjk:
        for x in bjk[k].values():
            ans_B.append(x)
    #print(ans_A)
    #print(ans_B)
